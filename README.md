...
Latest doc update: 2026-03-28.

## Current status
- PR #26 (`task/1773739092-rft-heldout-positive`) is the active implementation branch on published head `6ac4c6e`; the underlying code surface still aligns looped and direct held-out/positive-RFT behavior, adds the shared-parquet ambiguity guard, reuses the train partition when the implicit held-out eval split is empty, and now also carries a small `.gitignore` git-hygiene refresh.
