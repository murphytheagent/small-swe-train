...
- RFT/pilot sampling is code-driven: `src/rollout/onpolicy_collector.py` calls `load_task_batch(...)`, and `src/env/task_dataset.py` chooses tasks deterministically from the full train or held-out partition using `step_index`, `batch_size`, and wraparound. The experiment controls are therefore outer steps, task batch size, attempts per task, requested global SFT batch, topology-derived SFT row floor, and held-out eval count, and the RFT values must be defined through `configs/` rather than ad hoc launch-env tuning.
...
- Stale preflight jobs and artifacts from the stopped 4-GPU attempts were cleaned on 2026-04-29: previous `outputs/preflight_runs/20260428*` and `20260429T001254Z_4gpu_rft_min4` roots plus `psf-*` Slurm logs were removed, and stale pilot jobs `3036`-`3039` were canceled.
- Fresh 8-GPU preflights are queued under `outputs/preflight_runs/20260429T003423Z_8gpu_preflight_fresh` with format RFT selecting format-valid trajectories only, not terminal submissions. RFT jobs: `3252` format JSON, `3253` format XML, `3254` positive JSON after `3252`, and `3255` positive XML after `3253`. Teacher-reprompt pilot jobs: `3256`/`3257` JSON replicates and `3258`/`3259` XML replicates. Submission check showed all fresh preflight jobs pending behind the existing W17 probe queue.
...
- Monitor queued preflight matrix for runtime health and results. JSON remains the default contract unless matched JSON-vs-XML evidence says otherwise; XML is an explicit opt-in config for this comparison.
- Preflight 1: monitor real `format_rft` JSON/XML jobs with `Qwen/Qwen3-8B`, thinking off, `5` outer steps, `task_batch_size=512`, `samples_per_task=4`, requested global SFT batch `16`, `data.micro_batch_size_per_gpu=1`, topology-derived minimum selected rows `8` on 8 GPUs, and `eval_task_count=50`. Track selected-row count, format-valid rate, held-out resolved/selected counts, and whether held-out eval improves from step 0.
- Preflight 2: monitor dependent positive RFT JSON/XML jobs initialized from the matching format job manifest, with `5` outer steps and the same collection/eval settings. Treat failure to select at least `NNODES * NPROC_PER_NODE * data.micro_batch_size_per_gpu` verifier-positive rows or repeated skipped inner optimization steps as a hard blocker for full difficulty banding.
