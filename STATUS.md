...
- Replacement 4-GPU preflights first queued as `3024`-`3031` under `outputs/preflight_runs/20260428T092008Z_4gpu`, but those parent/pilot jobs exited immediately because Slurm executed the `--wrap` body with `/bin/sh` and rejected `set -o pipefail`; orphaned positive jobs `3026`/`3027` were canceled.
- Corrected 4-GPU preflights were queued on 2026-04-28 under `outputs/preflight_runs/20260428T211910Z_4gpu` with an explicit `bash -lc` wrapper: format RFT jobs `3032` JSON and `3033` XML, dependent positive RFT jobs `3034` JSON after `3032` and `3035` XML after `3033`, and teacher-reprompt pilot replicates `3036`/`3037` JSON plus `3038`/`3039` XML. Early startup check showed `3032`/`3033` running and vLLM serving chat completions, but the format jobs then failed before training because selected train rows after length filtering were below the old format min-row threshold (`28 < 32` JSON, `27 < 32` XML); orphaned positive jobs `3034`/`3035` were canceled.
- The format preflight configs now use `train_batch_size=train_min_rows=4`, matching the 4-GPU-safe positive-RFT floor. A fresh RFT-only retry was queued on 2026-04-29 under `outputs/preflight_runs/20260429T001254Z_4gpu_rft_min4`: format jobs `3248` JSON and `3249` XML, dependent positive jobs `3250` JSON after `3248` and `3251` XML after `3249`. At submission check these were pending behind the running JSON pilot jobs `3036`/`3037`, while XML pilot jobs `3038`/`3039` were still pending.
...
- Preflight 1: monitor real `format_rft` JSON/XML jobs with `Qwen/Qwen3-8B`, thinking off, `5` outer steps, `task_batch_size=512`, `samples_per_task=4`, `train_batch_size=train_min_rows=4`, and `eval_task_count=50`. Track selected-row count, format-valid rate, held-out resolved/selected counts, and whether held-out eval improves from step 0.
