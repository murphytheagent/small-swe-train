...
RFT_VLLM_GPU_MEMORY_UTILIZATION="${RFT_VLLM_GPU_MEMORY_UTILIZATION:-0.75}"
RFT_VLLM_MAX_NUM_SEQS="${RFT_VLLM_MAX_NUM_SEQS:-32}"
RFT_VLLM_MAX_NUM_BATCHED_TOKENS="${RFT_VLLM_MAX_NUM_BATCHED_TOKENS:-131072}"
...
if [[ -z "${RFT_VLLM_EXTRA_ARGS}" ]]; then
  ...
  RFT_VLLM_EXTRA_ARGS="${RFT_VLLM_EXTRA_ARGS} --max-num-seqs ${RFT_VLLM_MAX_NUM_SEQS}"
  RFT_VLLM_EXTRA_ARGS="${RFT_VLLM_EXTRA_ARGS} --max-num-batched-tokens ${RFT_VLLM_MAX_NUM_BATCHED_TOKENS}"
  RFT_VLLM_EXTRA_ARGS="${RFT_VLLM_EXTRA_ARGS} --default-chat-template-kwargs '{\"enable_thinking\": false}'"
fi
